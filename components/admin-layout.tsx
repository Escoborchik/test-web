								...
								return (
									<li key={item.name}>
										<Link
											href={item.href}
											className={cn(
												'group relative flex gap-x-3 rounded-lg p-2.5 text-sm font-medium leading-6 transition-colors',
												isActive
										...
										>
											{isActive && (
												<div className="absolute -left-4 top-1/2 -translate-y-1/2 w-1 h-8 bg-accent rounded-r-full" />
											)}
				...
				<header className="sticky top-0 z-40 flex h-14 shrink-0 items-center gap-x-4 border-b border-border bg-card px-4 sm:px-6 lg:px-8">
					...
					<div className="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
						<div className="flex flex-1 items-center">
							<span className="text-lg font-semibold text-primary lg:hidden">
								Book&Play
							</span>
						</div>
