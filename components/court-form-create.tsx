...
export const CourtFormCreate = ({ onSave, onCancel }: CourtFormCreateProps) => {
	...
	const [amountCourts, setAmountCourts] = useState(1);
	...
	const handleAddCourts = (courtInfo: Court, amount: number) => {
		for (let i = 0; i < amount; i++) {
			onSave(courtInfo);
			courtInfo = { ...courtInfo, id: crypto.randomUUID() };
		}
		...
		<div className="space-y-4 p-4 bg-white rounded-lg border-2 border-primary shadow-md">
			...
			<div className="flex gap-2 justify-end">
				...
				<Button
					...
					onClick={() => handleAddCourts(formData, amountCourts)}
				>
					Сохранить
				</Button>
			</div>
