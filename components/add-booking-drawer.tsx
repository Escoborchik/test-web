...
export function AddBookingDrawer({
  ...
  const addExtra = (extraId: string) => {
    setSelectedExtras((prevState) => [...prevState, { extraId, quantity: 1 }]);
  };
  ...
  const decrementService = (extraId: string) => {
    const extraForDeleting = selectedExtras.findIndex(
      (selectedExtra) =>
        selectedExtra.extraId === extraId && selectedExtra.quantity === 1
    );
  ...
  return (
    <>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="p-2 bg-muted/50 rounded">
                      ...
                      <p className="font-medium text-foreground mt-0.5">
                        {COVER_TYPE_LABELS[slot.coverType]}
                      </p>
                    </div>
                    <div className="p-2 bg-muted/50 rounded">
                      <p className="text-xs text-muted-foreground">Тип корта</p>
                      <p className="font-medium text-foreground mt-0.5">
                        {slot.isIndoor ? "Открытый" : "Закрытый"}
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="space-y-5 p-3 bg-accent/5 rounded-lg border border-accent/20">
                <div className="flex flex-col gap-2 text-sm">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setSelectedTariff(value)}
                  >
                ...
                <div className="flex flex-col gap-2 mb-2.5">
                  <Label
                    ...
                    className="text-sm flex items-center gap-2"
                  >
                    {isRecurring ? "Цена за одно занятие" : "Цена"}
                  </Label>
                          ...
                          <Popover>
                            ...
                            >
                              <Calendar
                                ...
                                selected={
                                  startDate
                                    ? parseLocalDate(startDate) ?? undefined
                                    : undefined
                                }
                                onSelect={(value) =>
                                  setStartDate(
                          ...
                          <Popover>
                            ...
                            >
                              <Calendar
                                ...
                                selected={
                                  endDate
                                    ? parseLocalDate(endDate) ?? undefined
                                    : undefined
                                }
                                onSelect={(value) =>
                                  setEndDate(
                          ...
                          <div className="p-2.5 bg-primary/10 rounded-lg border border-primary/30">
                            <p className="text-sm font-medium text-foreground">
                              ...
                              <span className="text-primary font-semibold">
                                {(totalPriceBooking ?? 0).toLocaleString(
                                  "ru-RU"
                                )}{" "}
                                ₽
                              </span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 p-3 bg-accent/5 rounded-lg border border-accent/20">
                    {extras.map((extra) => {
                      const quantity =
                        selectedExtras.find(
                          (selectedExtra) => selectedExtra.extraId === extra.id
                        )?.quantity || 0;
                      const extraPrice =
                        extra.unit === "hour"
                            ...
                            {quantity > 0 ? (
                              <>
                                <Button
                                  ...
                                  size="icon"
                                  className="h-7 w-7"
                                  onClick={() => decrementService(extra.id)}
                                >
                                ...
                                <Button
                                  ...
                                  disabled={quantity >= extra.amount}
                                >
                    ...
                    {totalBookings > 0 && extraPricePerSingle > 0 && (
                      <div className="space-y-2 p-3 pt-2">
                        <div className="p-2 bg-accent/10 rounded-lg border border-accent/20">
                          <p className="text-sm font-medium text-foreground">
                            Итого услуги (за 1 занятие):{" "}
                            <span className="text-accent">
                              {extraPricePerSingle}
                            </span>
                          </p>
                        ...
                        <div className="p-2.5 bg-primary/10 rounded-lg border border-primary/30">
                          <p className="text-sm font-medium text-foreground">
                            Итоговая сумма:{" "}
                            <span className="text-primary font-semibold">
                              {(totalExtraPrice ?? 0).toLocaleString("ru-RU")} ₽
                            </span>
                          </p>
                        </div>
                      </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {selectedExtras.length > 0 && (
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-sm font-medium text-foreground pt-0.5">
                        <span>Услуги:</span>
                        <span>{totalExtraPrice.toLocaleString()} ₽</span>
                      </div>
                      {selectedExtras.map(({ extraId, quantity }) => {
                        const extra = extras.find((ext) => ext.id === extraId);
                        ...
                        const extraPrice =
                          extra.unit === "hour"
                            ...
                            : extra.price;
                        const totalExtraPrice = isRecurring
                          ? extraPrice * quantity * calculateTotalBookings()
                          : extraPrice * quantity;
                  ...
                  <div className="pt-1.5 mt-1.5 border-t border-primary/20 flex items-center justify-between">
                    ...
                    <span className="text-base font-bold text-primary">
                      {(totalExtraPrice + totalPriceBooking).toLocaleString()} ₽
                    </span>
                  </div>
                </div>
              </div>
      ...
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Вы уверены, что хотите добавить это бронирование? Клиент получит
              уведомление.
            </DialogDescription>
          </DialogHeader>
          <div className="flex gap-3 justify-end mt-4">
            ...
            <Button
              ...
              onClick={() => {
                ...
                const baseDateForPrice = bookingDates[0] || slot.date;

                const pricePerHour = (() => {
                  if (!selectedCourt || !baseDateForPrice) return 0;

                  const dayGroup = isWeekendDate(baseDateForPrice)
                    ? "weekends"
                    : "weekdays";
                  const priceSlots = selectedCourt.prices?.[dayGroup] ?? [];

                  const startMinutes = timeStringToMinutes(startTime);

                  const matchedSlot = priceSlots.find(({ from, to }) => {
                ...
                const price = Math.round(durationHours * pricePerHour);
                ...
                if (isRecurring) {
                  const recurringStart = startDate || bookingDates[0] || "";
                  const recurringEnd =
                    endDate ||
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Отменить добавление бронирования?</DialogTitle>
            <DialogDescription>
              Если вы отмените, все введённые данные будут потеряны. Вы уверены,
              что хотите прервать заполнение?
            </DialogDescription>
          </DialogHeader>
          <div className="flex gap-3 justify-end mt-4">
            <Button
